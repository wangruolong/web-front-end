        ...
        rules: [
            {
                test: /\.(scss|css)$/,
                use: [
                    // MiniCssExtractPlugin.loader和style-loader二选一，他们的区别是。
                    // 相同之处：
                    // 1.他们都是把css-loader加载后的样式文件独立出来。
                    // 不同之处：
                    // 1.独立方式不一样。MiniCssExtractPlugin.loader，把css文件打包成<link />的形式诸如到页面中的<head />里面；style-loader把css文件打包成以<style />的形式注入到页面中<head />里面。
                    // 2.返回时间点不一样。<link />和<style />的区别是前者需要再请求一次，后者跟着页面一起返回不用再请求一次。
                    // 3.应用场景不一样。如果页面文件比较大可以用<link />的方式进行分次请求就不用一次请求很久才回来，如果页面文件比较小可以用style和页面一起返回就只要请求一次就可以了不用请求多次。
                    ...
                    {
						loader: 'css-loader',// css-loader用来处理import Styles from 'xxx.css'语法导入的文件。
						options: {
                ...
                use: [
                    {
                        loader:'file-loader',
                        options: {
                            outputPath:'assets/'// 把打包的资源文件放到assets里面
                        }
				    }
                ]
            },
