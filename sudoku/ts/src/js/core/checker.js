...
class Checker {
    ...
    checkBoxes() {
        for (let boxIndex = 0; boxIndex < 9; boxIndex++) {
            const boxes = toolkit_1.default.box.getBoxCells(this._matrix, boxIndex); //在矩阵中根据矩阵和宫序号获取宫
            const marks = checkArray(boxes); //检查这一宫的数据是否正确
            for (let cellIndex = 0; cellIndex < 9; cellIndex++) {
                if (!marks[cellIndex]) {
                    const { rowIndex, colIndex } = toolkit_1.default.box.convertFromBoxIndex(boxIndex, cellIndex); //把宫内序号转换成行列坐标
